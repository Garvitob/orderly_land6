...
const byTag = (tag: string): readonly Dish[] =>
  MENU.filter((d) => d.tags.includes(tag));
...
const INTENTS: readonly { intent: Intent; keywords: readonly string[] }[] = [
  ...
  { intent: "glutenfree", keywords: ["gluten", "gf", "wheat", "bun"] },
  { intent: "mild", keywords: ["mild", "not spicy", "no heat", "not too hot", "gentle", "for my mom", "for my kid", "child", "plain"] },
  { intent: "spicy", keywords: ["spicy", "spice", "hot", "heat", "cayenne", "chili", "chilli", "fiery"] },
  { intent: "vegetarian", keywords: ["vegetarian", "veggie", "veg", "no meat", "meatless", "vegan"] },
  { intent: "drink", keywords: ["drink", "shake", "coke", "soda", "thirsty", "beverage", "water", "malt"] },
  { intent: "dessert", keywords: ["dessert", "sweet", "pie", "afters", "ice cream"] },
  { intent: "price", keywords: ["price", "cost", "how much", "cheap", "expensive", "budget"] },
  { intent: "modify", keywords: ["without", "no onion", "extra", "on the side", "swap", "change", "instead of", "modify", "well done"] },
...
const REPLIES: Record<Intent, () => Reply> = {
  recommend: () => ({
    ...
    dishes: pick("smash-burger", "nashville-hot"),
  }),
  glutenfree: () => ({
    text: "The farm salad is naturally gluten free, and the Nashville hot without the bun. I would skip the cornbread.",
    dishes: pick("farm-salad", "nashville-hot"),
    chip: "Gluten free",
  }),
  allergy: () => ({
  ...
  spicy: () => ({
    text: "Nashville hot chicken, cayenne butter. That is the one with real heat.",
    dishes: pick("nashville-hot"),
    chip: "Spicy",
  }),
  vegetarian: () => ({
    text: "Plenty. Mac and cheese, the farm salad, or skillet cornbread if you want something warm.",
    dishes: byTag("vegetarian").slice(0, 3),
    chip: "Vegetarian",
  }),
  drink: () => ({
    text: "The vanilla shake is made with real vanilla and it is thick. There is chilled Coke if you would rather.",
    dishes: pick("vanilla-shake", "coke"),
  }),
  dessert: () => ({
    text: "Apple pie, warm, with a cinnamon crust. One slice is enough for two.",
    dishes: pick("apple-pie"),
  }),
  price: () => ({
